...
fn perform_match(bank: &mut ReplayBank, event_json_model: EventJsonModel) {
    // here replay for one user or all and print intermediate result
    match event_json_model.event_type {
        EventType::Deposit => {
            println!("Replaying step {}, user {} ...", event_json_model.step, event_json_model.user);
        ...
        EventType::BankSnapshot => {
            println!("Snapshot, user {} ...", event_json_model.user);
